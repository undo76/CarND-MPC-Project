...
SolveResult MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  ret.a =  solution.x[a_start];
  ret.x = vector<double>(N - 1);
  std::copy(&solution.x[x_start + 1], &solution.x[x_start + N], ret.x.begin());
  ret.y = vector<double>(N - 1);
  std::copy(&solution.x[y_start + 1], &solution.x[y_start + N], ret.y.begin());
  return ret;
}